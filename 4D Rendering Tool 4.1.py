def get_options():
    ...
    fil = open(filename, 'r')
    #Split 1st line in file by "|" and add to list of preset shapes
    options = fil.readline()[:-1].split("|")
    #add two additional items to list
    options += ["Shape Builder","Sphere Builder"]
    #Slpit 2nd line into list of rotations
    prefabs = fil.readline()[:-1].split("|")
    #cast rotations to floating point numbers
...
def change():
    ...
    #set current object selection (v) to tag
    tag = options[v.get()]
    ...
    line=0
    for ln in fil:
        #find the line in the file corresponding to the tag
        if ln[:-1] == tag:
            line=1
        elif line == 1:
            #The next line will be the coordinates for all the points
            obj = ln[:-1].split("|")
            for x in range(len(obj)):
                obj[x]= map(float,obj[x].split(","))
            line=2
        elif line ==2 :
            #the corresponding line after is the lines that connect the points
            rel = ln[:-1].split("|")
            #some shapes don't have lines
            if rel == ['']:
                rel = []
            else:
            ...
            #the final line stores faces that connect the points
            faces = ln[:-1].split("|")
            print faces
            #not all objects have (4 point) faces
            if faces == ['']:
                faces = []
            else:
...
def keypress(event):
    ...
    #add key that was pressed to dictionary (pressed)
    pressed[str(event.keysym)] = str(event.char)
    #Forward key input to a GUI if it's enabled
    ...
    if FGUI:
        ...
        return
    #changes position of object on screen / The 4D effect
    if event.keysym in Dctn:
        #how close object is to lightsouce
        pos[Dctn[event.keysym][0]] += Dctn[event.keysym][1]
        #how far sheet or the shadow is from lightsource
    ...
    #rotates bject to preset rotation
...
def keyrelease(event):
    #deletes key from the "pressed" dictionary
...
def handleroat():
    ...
    for Hash in pressed:
        #rotates object using user input
        if Hash in controls:
            index =  controls[Hash]
            rotation[index[0]] += index[1]*speed
        #sets rotation to preset one
        if Hash in presets:
            rotation = []+ presets[Hash]
        #moves point(s) on object
        if Hash in movement:
            index =  movement[Hash]
...
def maths(a,b, theta):
    ...
    return newa,newb
def rotate(x,y,z,w):
    #defines all the points to be repositioned
    ...
    return [x,y,z,w]
def move(item):
    #sets point's distance to lightsource based on object position
...
def line_rend():
    ...
    #time.sleep(0.010)
    tobe = []
    RGB = (R.get()/10.0,G.get()/10.0,B.get()/10.0)
    #retrieve colour values from slider
    for idx in range(len(nobj)):
        #cycles through each pair of points
        for x in rel[idx]:
            num = (-nobj[idx][2]- nobj[x][2]+contr[0]/2)/contr[1]
            #avg 3D distance from camera between the 2 points
    ...
    for tob in tobe:
        #makes sure distance of each line is within range
        if 100-int(tob[0]*10) >99:
            ...
            c.create_text(0,25,text="NO MORE CONTRAST",tag="tes",
            fill = "red",anchor=SW)
        #creates colour based on rgb and scales it based on distance to camera
        mycolour = '#%02x%02x%02x' % (tob[0]*RGB[0],tob[0]*RGB[1],tob[0]*RGB[2])
        line = c.create_line(tob[1],tob[2],tob[3],
            tob[4],tag="tes",width=tob[0],
            fill = mycolour)#"gray"+ str(int(tob[0]*10))) #str(100-int(tob[0]*10)))
    if var3.get():
        tobe = []
        for idx in range(len(nobj)):
...
def integrend():
    ...
    for idx in range(len(obj)):
        for x in rel[idx]:
            num = (-nobj[idx][2]- nobj[x][2]+contr[0]/2)/contr[1]
            tobe +=[[num,nobj[idx][0],nobj[idx][1],nobj[x][0],nobj[x][1],
            (nobj[idx][3]+nobj[x][3])/2,[0,0,25,4]]]
    #for idx in range(len(nobj)):
    for idx in range(len(obj)):
        num = (-nobj[idx][2]*2+contr[0]/2)/contr[1]
    ...
    for face in faces:
        for x in range(4):
            ...
            ,nobj[face[n2]][1],(nobj[face[x]][3]+nobj[face[n2]][3])/2,[25,0,0,0]]]
    tobe = sorted(tobe, key=itemgetter(5))
    for tob in tobe:
        if 100-int(tob[0]*10) >99:
            tob[0] = 1
            ...
            fill = "red",anchor=SW)
        if tob[6] != "Sph":
            mycolour = '#%02x%02x%02x' % (tob[6][0]*tob[0],
            tob[6][1]*tob[0],tob[6][2]*tob[0])
            line = c.create_line(tob[1],tob[2],tob[3],
            tob[4],tag="tes",width=tob[0]+tob[6][3],
            fill = mycolour)#tob[6])#"gray"+ str(100-int(tob[0]*10)))
        else:
...
def print_controls():
    global movement,controls,speedset,contrastop
    rot_axis = ["(4D)YW","(4D)XW","(3D)YZ","(3D)XZ","(4D)ZW","(2D)XY"]
    axis = ["X","Y","Z","W"]
    ctlvar = """\n\n\nShape Edits:
    \tLeft Mouse (on blank space): \tAdd point.
    \tLeft Mouse (on point): \tSelect point.
    \tLeft Mouse + Shift: \t\tCreate Line.
    \tLeft Mouse + ALt: \t\tCreate Face.
    \tRight Mouse (on point): \tDelete point.
    \tRight Mouse + Shift: \tDelete Line.
    \tRight Mouse + ALt: \t\tDelete Face.
    \tv: \t\t\tSave Shape"""
    controlvar ="\n\tControls menu: c \n"
    controlvar += "\tPresets: 1-9 \n"
    controlvar += "\tRotation: \n"
    for a in controls:
        controlvar += "\t\t%s: %d degree about the %s axis. \n" %(
            a, controls[a][1],rot_axis[controls[a][0]])
    ctlvar += "\nPoint Translation: \n"
    for a in movement:
        ctlvar += "\t%s: %d unit in the %s axis. \n" %(
            a, movement[a][1],axis[movement[a][0]])
    controlvar += "\tSpeed: \n"
    for a in speedset:
        if not speedset[a][1]:
            controlvar += "\t\t(%s): Reset Speed\n" %a
            continue
        controlvar += "\t\t(%s): Add %.2f speed\n" %(a, speedset[a][0])
    controlvar += "\tContrast: \n"
    for a in contrastop:
        str1=("%.2f contrast."%contrastop[a][0])*(not not contrastop[a][0])
        str1+=("%.2f brightness."%contrastop[a][1])*(not not contrastop[a][1])
        controlvar += "\t\t(%s): Add %s\n" %(a,str1)
    c.create_text(0, 0,fill = 'blue',
    text=controlvar,tag = "tes",anchor=NW,font = ('Times', 10, 'bold'))
    c.create_text(310, 0,fill = 'blue',
    text=ctlvar,tag = "tes",anchor=NW,font = ('Times', 10, 'bold'))
#-------------------------------------------------------------------------------
from Tkinter import *
...
v2,R,G,B = IntVar(),IntVar(),IntVar(),IntVar()
CurItm = 0
...
Scale(root,from_=255,to=0,variable=R).pack(side="left")
Scale(root,from_=255,to=0,variable=G).pack(side="left")
Scale(root,from_=255,to=0,variable=B).pack(side="left")
Scale(root,from_=50,to=1500,command=scaleit,orient=HORIZONTAL).pack(side="left")
#initface()
B.set(255)
R.set(255)
var.set(True)
