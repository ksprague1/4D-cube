...
for s4 in range(int(sph**0.5*2/index + 2)):
    ...
    if s4 == int(sph**0.5*2/index + 2)-1:
        num4 = (sph)**0.5
    down4 = num4 ** 2
    for s3 in range(int((sph- down4)**0.5*2/index + 2)):
        #print (360.0/int((sph- down4)**0.5*2/index + 1)*s3/4)
        num3 = (sph- down4)**0.5*math.sin(math.radians(360.0/int((sph- down4)
        **0.5*2/index + 1)*s3/4))
        if s3 == int((sph- down4)**0.5*2/index + 2)-1:
            if s4 == int(sph**0.5*2/index + 2)-1:
                print "GOT EM"
                continue
            num3 = (sph-down4)**0.5
...
while True:
    ...
    for tob in range(len(tobe)):
        ...
        line = c.create_text(tobe[tob][1],tobe[tob][2],text = str(tob),tag="tes")
        if "m" in pressed:
